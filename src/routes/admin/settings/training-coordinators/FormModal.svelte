...
>
	<form use:form>
		...
		<TextInput invalid={$errors.phone} name="phone" labelText="phone" placeholder="Enter  phone..." />
		{#if $errors.phone}
			<p class="t-text-red-500">{$errors.phone}</p>
		{/if}
		<!-- <TextInput name="alt_phone" labelText="alt_phone" placeholder="Enter  alt_phone..." /> -->
		<TextInput invalid={$errors.email} name="email" labelText="Email" placeholder="Enter  Email..." />
		{#if $errors.email}
